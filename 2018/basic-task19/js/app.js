window.addEventListener('load', function(){
    ...
    //增加记录用户选择历史记录的功能，使用BOM的 history对象实现
    (function(){
        function getStr(){
        ...
        function setStr(){
            var str = '?q=',
                title = '',
                tempStr = '';
            if(product_checkbox.inputsData[product_checkbox.inputsData.length - 1]
               && region_checkbox.inputsData[region_checkbox.inputsData.length - 1]){
                window.history.pushState({}, document.title + '全部地区，全部商品', '?q=all');
            }else{
                region_checkbox.inputsData.forEach(function(item, idx, arr){
                    if(item && item !== 'all') tempStr += item; 
                });
                title = '地区：' + tempStr + ' ';
                str += tempStr;
                tempStr = '';
                product_checkbox.inputsData.forEach(function(item, idx, arr){
                    if(item && item !== 'all') tempStr += item; 
                });
                title = '商品：' + tempStr;
                str += tempStr;
                tempStr = '';
                window.history.pushState({}, title, str);
            }
        }
        ...
        if(window.history.pushState){
            ...
            product_checkbox.container.addEventListener('click', setStr, false);
            region_checkbox.container.addEventListener('click', setStr, false);
            anaStr();
        }
    })();
},false);